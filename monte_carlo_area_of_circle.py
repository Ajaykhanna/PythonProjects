...
The number 4 in the area_estimate equation arises from the relationship between the area of the circle and the area of
the square that we use in the Monte Carlo simulation.

Let me break it down:
Square Area: The square that encompasses the circle has a side length of 2 times the radius (to fully contain the circle).
Therefore, the area of the square is calculated as (2 * radius)**2, which simplifies to 4 * radius**2.
Ratio of Areas: The Monte Carlo method essentially compares the number of points that fall inside the circle to the total
number of points generated within the square. This ratio approximates the ratio of the circle's area to the square's area.
Estimated Circle Area: To obtain the estimated area of the circle, we take the ratio of points inside the circle to the total
points and multiply it by the area of the square. This is where the 4 comes into play:
area_estimate = (in_circle / num_points) * (4 * radius**2)
Use code with caution.
In essence, we are scaling the ratio of points by the area of the square to get an estimate of the circle's area.
The 4 is a factor that accounts for the difference in area between the circle and the square we use for the simulation.
"""
